    ...
    computed: {
            resolvedImageUrl() {
                if (this.restItem?.image) {
                    return new URL(`/src/assets/images/rests/${this.restItem.image}`, import.meta.url).href; // абсолютный путь
                }
    return '';
            },
            vectorIconUrl() {
                return new URL('/src/assets/images/Vector.svg', import.meta.url).href;
            }
    }
}
</script>